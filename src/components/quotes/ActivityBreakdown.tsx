  ...
  return (
    <div className="space-y-4">
      ...
      <button
        type="button"
        onClick={openModal}
        className="w-full rounded-lg border-2 border-dashed border-line px-4 py-3 text-left transition-colors hover:border-brand hover:bg-brand/5"
      >
      ...
      <Modal
        open={isModalOpen}
        title="Sélectionner les activités"
        size="lg"
      ...
      >
        ...
        <div className="mt-4 max-h-72 space-y-1 overflow-y-auto">
          {filteredOptions.map((option) => (
            <label
              key={option.value}
              className="flex cursor-pointer items-center rounded-md p-2 hover:bg-surface"
            >
              <input
                type="checkbox"
              ...
              />
              <span className="ml-3 text-sm text-ink">{option.label}</span>
            </label>
          ))}
          {filteredOptions.length === 0 ? (
            <EmptyState title="Aucune activité ne correspond" />
          ) : null}
        </div>
      </Modal>
    </div>
